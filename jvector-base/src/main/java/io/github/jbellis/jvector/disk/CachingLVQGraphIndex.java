...
public class CachingLVQGraphIndex implements GraphIndex, AutoCloseable, Accountable
{
    private static final int CACHE_DISTANCE = 3;

    private final LVQGraphCache cache;
    public final OnDiskLVQGraphIndex graph;

    public CachingLVQGraphIndex(OnDiskLVQGraphIndex graph)
    {
        this(graph, CACHE_DISTANCE);
    }

    public CachingLVQGraphIndex(OnDiskLVQGraphIndex graph, int cacheDistance)
    {
        this.graph = graph;
        this.cache = LVQGraphCache.load(graph, cacheDistance);
    }
    ...
    public String toString() {
        return String.format("CachingLVQGraphIndex(graph=%s)", graph);
    }

    private class CachedView implements LVQView {
        private final OnDiskLVQGraphIndex.OnDiskView view;

        public CachedView(OnDiskLVQGraphIndex.OnDiskView view) {
            this.view = view;
        }
        ...
        public LocallyAdaptiveVectorQuantization.PackedVector getPackedVector(int node) {
            return view.getPackedVector(node);
        }
