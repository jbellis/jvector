...
public class NodeQueue {
    ...
    /**
     * Apply the given consumer to the top 2 nodes in the heap without removing them.
     * If the heap has fewer than 2 elements, only those elements will be processed.
     * The nodes are processed in order from highest to lowest score.
     *
     * @param consumer the consumer to process each node ID
     */
    public void forEachTop3(IntIntConsumer consumer) {
        if (heap.size() == 0) {
            return;
        }
        // Process root node (index 1)
        consumer.accept(0, decodeNodeId(heap.get(1)));
        if (heap.size() == 1) {
            return;
        }

        // If we only have a left child, use it
        if (heap.size() == 2) {
            consumer.accept(1, decodeNodeId(heap.get(2)));
            return;
        }

        // Compare children to find the second highest
        int leftChild = 2;
        int rightChild = 3;
        long leftRaw = heap.get(leftChild);
        long rightRaw = heap.get(rightChild);

        // Apply order transformation to compare scores correctly
        long leftValue = order.apply(leftRaw);
        long rightValue = order.apply(rightRaw);

        if (leftValue >= rightValue) {
            consumer.accept(1, decodeNodeId(leftRaw));
            findThirdPlace(consumer, leftChild, rightChild, rightRaw);
        } else {
            consumer.accept(1, decodeNodeId(rightRaw));
            findThirdPlace(consumer, rightChild, leftChild, leftRaw);
        }
    }

    private void findThirdPlace(IntIntConsumer consumer, int secondPlace, int thirdCandidate, long candidateRaw) {
        int leftGrandChild = secondPlace * 2;
        int rightGrandChild = secondPlace * 2 + 1;
        long bestRaw = candidateRaw;
        int bestNode = thirdCandidate;

        if (leftGrandChild <= heap.size()) {
            long leftRaw = heap.get(leftGrandChild);
            if (order.apply(leftRaw) > order.apply(bestRaw)) {
                bestRaw = leftRaw;
                bestNode = leftGrandChild;
            }
        }
        if (rightGrandChild <= heap.size()) {
            long rightRaw = heap.get(rightGrandChild);
            if (order.apply(rightRaw) > order.apply(bestRaw)) {
                bestNode = rightGrandChild;
            }
        }
        consumer.accept(2, decodeNodeId(heap.get(bestNode)));
    }
}
