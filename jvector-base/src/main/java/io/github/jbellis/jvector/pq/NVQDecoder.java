 ...
 */
abstract class NVQDecoder implements ScoreFunction.ApproximateScoreFunction {
    private static final VectorTypeSupport vts = VectorizationProvider.getInstance().getVectorTypeSupport();

    protected final NVQVectors cv;

    protected NVQDecoder(NVQVectors cv) {
        this.cv = cv;
    }

    protected static abstract class CachingDecoder extends NVQDecoder {
        protected final VectorFloat<?> partialSums;

        protected CachingDecoder(NVQVectors cv, VectorFloat<?> query, VectorSimilarityFunction vsf) {
            super(cv);
            var pq = this.cv.pq;
    ...
    static class DotProductDecoder extends CachingDecoder {
        public DotProductDecoder(NVQVectors cv, VectorFloat<?> query) {
    ...
    static class EuclideanDecoder extends CachingDecoder {
        public EuclideanDecoder(NVQVectors cv, VectorFloat<?> query) {
    ...
    static class CosineDecoder extends NVQDecoder {
        protected final VectorFloat<?> partialSums;
        protected final VectorFloat<?> aMagnitude;
        protected final float bMagnitude;

        public CosineDecoder(NVQVectors cv, VectorFloat<?> query) {
            super(cv);
            var pq = this.cv.pq;
