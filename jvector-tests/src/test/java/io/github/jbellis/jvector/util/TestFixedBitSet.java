...
public class TestFixedBitSet extends BaseBitSetTestCase<FixedBitSet> {
  ...
  void doRandomSets(int maxSize, int iter, int mode) throws IOException {
    ...
    for (int i = 0; i < iter; i++) {
      ...
      // benchmark the various ways of setting bits
      if (sz > 0) {
  ...
  */
  ...
  public void testIntersectionCount() {
    ...
    // If ghost bits are present, these may fail too, but that's not what we want to demonstrate
    // here
    // Assert.assertEquals(bitSet1.cardinality(), fixedBitSet1.cardinality());
    // Assert.assertEquals(bitSet2.cardinality(), fixedBitSet2.cardinality());
